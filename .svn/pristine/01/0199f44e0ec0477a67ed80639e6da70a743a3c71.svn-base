...
import model.select.selectCourse;
import model.select.selectGeneration;

/**
 * Servlet implementation class ListGen
 */
@WebServlet("/admin/listActiveCourse.hrd")
public class ListActiveCourse extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public ListActiveCourse() {
		super();
	...
	public void doProcess(HttpServletRequest request, HttpServletResponse response) throws Exception {
		try {
			...
			String gen = utils.Convertor.convertResultSetIntoJSON(lst.SelectActiveCourse()).toString();
			lst.close();
