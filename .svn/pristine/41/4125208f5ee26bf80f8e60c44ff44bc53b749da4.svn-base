...
<header class="main-header">
	<!-- Logo -->
	<a href="index.jsp" class="logo" style="position: fixed;"> <!-- mini logo for sidebar mini 50x50 pixels -->
		<span class="logo-mini"><b>S</b>M</span> <!-- logo for regular state and mobile devices -->
		<span class="logo-lg"><img class="left-logo" src="image/logo.png"></span>
	</a>
			...
			<ul class="nav navbar-nav">
				...
				<li class="dropdown user user-menu"><a href="#"
					class="dropdown-toggle" data-toggle="dropdown"> <img
						src="../image/staff/<%=((StaffDTO) session.getAttribute("user")).getPhoto()%>" class="user-image" alt="User Image" />
						<span class="hidden-xs"> 
							<%=((StaffDTO) session.getAttribute("user")).getFirstName()%>
							<%=((StaffDTO) session.getAttribute("user")).getLastName()%>
					</span>
				</a>
					<ul class="dropdown-menu">
						<!-- User image -->
						<li class="user-header"><img
							src="../image/staff/<%=((StaffDTO) session.getAttribute("user")).getPhoto()%>" class="img-circle"
							alt="User Image" />
							<p>
								<%=((StaffDTO) session.getAttribute("user")).getFirstName()%>
								<%=((StaffDTO) session.getAttribute("user")).getLastName()%>
								...
								-
								<%=((StaffDTO) session.getAttribute("user")).getPosition()%>
								<small> <%=((StaffDTO) session.getAttribute("user")).getEmail()%>
								</small>
							</p></li>
						<!-- Menu Body -->
						<!-- <li class="user-body">
